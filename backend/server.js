...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    if (!user.lastLogin) {
      // New user's very first login - Start from 1
      user.loginStreak = 1;
    } else {
      ...
      // If dayDiff is 0 (already logged in today), we keep the current streak
    }
...
app.get('/api/content', async (req, res) => {
  ...
  try {
    ...
    // Auto-populate missing summaries for the retrieved content
    if (contents.length > 0) {
      const missingSummaries = contents.filter(c => !c.summary);
      if (missingSummaries.length > 0) {
        await Content.updateMany(
          { _id: { $in: missingSummaries.map(c => c._id) } },
          { $set: { summary: "This comprehensive lesson provides a structured breakdown of the topic, including key concepts, logical flows, and practical examples to ensure deep understanding." } }
        );
        // Update the local array as well so the user sees it immediately
        missingSummaries.forEach(c => c.summary = "This comprehensive lesson provides a structured breakdown of the topic, including key concepts, logical flows, and practical examples to ensure deep understanding.");
      }
    }
...
app.get('/api/content/:id', async (req, res) => {
  try {
    ...
    // Fix: If summary is missing or empty, update it before sending
    if (!content.summary || content.summary.trim() === '') {
      content.summary = "This comprehensive lesson provides a structured breakdown of the topic, including key concepts, logical flows, and practical examples to ensure deep understanding.";
      await content.save();
    }
    
    // Ensure summary is part of the data object for frontend
    const responseData = {
      ...content.toObject(),
      ...content.data,
      summary: content.summary // Explicitly set summary at top level
    };
...
app.get('/api/search', async (req, res) => {
  ...
  try {
    ...
    // Update summaries for these topics if missing
    if (contents.length > 0) {
      await Content.updateMany(
        { _id: { $in: contents.map(c => c._id) }, summary: { $exists: false } },
        { $set: { summary: "This comprehensive lesson provides a structured breakdown of the topic, including key concepts, logical flows, and practical examples to ensure deep understanding." } }
      );
    }
...
// Route to randomly set streaks for all users (1-7 days)
app.post('/api/admin/randomize-streaks', auth, adminOnly, async (req, res) => {
  try {
    const users = await User.find({});
    const updates = users.map(user => {
      const randomStreak = Math.floor(Math.random() * 7) + 1;
      const lastLogin = new Date();
      // Set last login to today or yesterday randomly to maintain the streak
      if (Math.random() > 0.5) {
    ...
    res.json({ success: true, message: `Streaks randomized for ${users.length} users (1-7 days)` });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});

// Route to promote a user to admin (Secure this or use once)
app.post('/api/admin/promote-specific-user', async (req, res) => {
  try {
    const email = 'puneetadmin031@gmail.com';
    ...
    res.json({ success: true, message: `${email} is now an admin`, user: { email: user.email, role: user.role } });
  } catch (error) {
...
app.get('/api/videos/search', auth, async (req, res) => {
  try {
    ...
    // Use a free-tier/public API or a library to search YouTube
    // For now, we'll use a library that scrapes/proxies public YouTube results
    const youtubesearchapi = require('youtube-search-api');
    const results = await youtubesearchapi.GetListByKeyword(q, false, 3); // Get top 3 videos
    ...
    const videos = results.items
      ...
      .map(item => ({
        ...
        title: item.title,
        thumbnail: item.thumbnail?.thumbnails?.[0]?.url || `https://img.youtube.com/vi/${item.id?.videoId || item.id}/hqdefault.jpg`,
...
app.get('/api/admin/analytics', auth, adminOnly, async (req, res) => {
  try {
    // Update Content with summaries if missing (One-time fix for existing content)
    await Content.updateMany(
    ...
    // Daily activity (last 7 days) - Combines Quizzes, Searches, and Logins
    const todayEnd = new Date();
    todayEnd.setHours(23, 59, 59, 999);
    
    // Use local time for generating the 7-day range to match today's date correctly
    const dailyActivity = [];
    for (let i = 6; i >= 0; i--) {
      const d = new Date();
    ...
    const [quizActivity, searchActivity, loginActivity] = await Promise.all([
      QuizAttempt.aggregate([
        { $match: { createdAt: { $gte: sevenDaysAgoStart, $lte: todayEnd } } },
      ...
      User.aggregate([
        { $match: { lastLogin: { $gte: sevenDaysAgoStart, $lte: todayEnd } } },
        { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$lastLogin" } }, count: { $sum: 1 } } }
      ])
    ]);

    // Update the dailyActivity array with actual counts
    dailyActivity.forEach(day => {
      const qCount = quizActivity.find(a => a._id === day._id)?.count || 0;
      const sCount = searchActivity.find(a => a._id === day._id)?.count || 0;
      const lCount = loginActivity.find(a => a._id === day._id)?.count || 0;
      day.count = qCount + sCount + lCount;
    });
    ...
    // Detailed user list with activity
    const usersWithActivity = await Promise.all(students.map(async (student) => {
      const userQuizzes = await QuizAttempt.find({ user: student._id }).sort({ createdAt: -1 });
      const userSearches = await SearchHistory.find({ user: student._id }).sort({ createdAt: -1 });
      
      // Use the new login-based streak
      const streak = student.loginStreak || 0;
      
      const avgScore = userQuizzes.length > 0 
        ? userQuizzes.reduce((sum, q) => sum + q.accuracy, 0) / userQuizzes.length 
        : 0;
      ...
      return {
        ...
        activity: {
          ...
          searchedTopics: [...new Set(userSearches.map(s => s.topic))].slice(0, 10),
          recentQuizzes: userQuizzes.slice(0, 5).map(q => ({
            topic: q.topic,
            score: q.accuracy,
            date: q.createdAt
          }))
    ...
    res.json({
      ...
      data: {
        stats: {
          ...
          avgQuizzesPerUser: totalUsers > 0 ? Math.round(totalQuizzes / totalUsers) : 0,
...
app.get('/api/user/dashboard', auth, async (req, res) => {
  try {
    ...
    const [quizAttempts, learningProgress, flashcardStats, dbUser, certificates, searchHistory] = await Promise.all([
    ...
    // Login-based Streak Calculation
    let streak = dbUser.loginStreak || 0;
    
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const lastLogin = new Date(dbUser.lastLogin || dbUser.createdAt);
    lastLogin.setHours(0, 0, 0, 0);

    // If the last login was more than 1 day ago, the streak is technically broken 
    // but we show the last recorded streak until they login again next day.
    // The actual reset happens at login time.
    ...
    const dbRecentProgress = await LearningProgress.find({ user: userId })
      .sort({ lastStudied: -1 })
      .limit(5);

    // Merge with recent searches to ensure "Continue Learning" is populated
    const progressTopics = new Set(dbRecentProgress.map(p => p.topic.toLowerCase().trim()));
    const recentProgress = [...dbRecentProgress];

    // Add missing topics from search history
    const topSearches = searchHistory
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
      .slice(0, 5);

    for (const search of topSearches) {
      const topic = (search.topic || search.query).trim();
      if (!progressTopics.has(topic.toLowerCase()) && recentProgress.length < 5) {
        recentProgress.push({
          topic: topic,
          progress: 0,
          status: 'started',
          lastStudied: search.createdAt,
          _id: `temp-${search._id}`
        });
        progressTopics.add(topic.toLowerCase());
      }
    }
...
app.post('/api/learn/query', auth, async (req, res) => {
  ...
  try {
    ...
    // 1. Save to SearchHistory and Initialize LearningProgress
    try {
      ...
      // Initialize LearningProgress for "Continue Learning" section
      await LearningProgress.findOneAndUpdate(
        { user: userId, topic: query.trim() },
        { 
          $setOnInsert: { 
            progress: 0, 
            status: 'started',
      ...
      console.warn('⚠️ Failed to save search history or progress:', historyErr.message);
    }
    ...
    if (cachedContent) {
      ...
      // Ensure summary is included in the returned data
      const responseData = {
        ...cachedContent.toObject(),
        ...cachedContent.data,
        cached: true
      };

      if (!responseData.summary || responseData.summary.trim() === '') {
        responseData.summary = cachedContent.summary || "This comprehensive lesson provides a structured breakdown of the topic, including key concepts, logical flows, and practical examples to ensure deep understanding.";
      }
    ...
    try {
      ...
      const newCache = new Content({
        ...
        summary: result.summary || "This comprehensive lesson provides a structured breakdown of the topic, including key concepts, logical flows, and practical examples to ensure deep understanding.",
        data: result,
        isPublic: true
      });
